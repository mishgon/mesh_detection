...
class BitNet(nn.Module):
    def __init__(self, structure, n_points, conv_block, pooling):
        ...
        pooling: Callable
            ``pooling()`` returns pooling layer. For example nn.MaxPool2d
    ...
    def softargmax(x):
        ...
        """
        batch_size, n_points, *spatial = x.shape
        coords = torch.stack(torch.meshgrid(*map(torch.arange, spatial)))
        softmax = torch.softmax(x.reshape(batch_size, n_points, -1), dim=-1).reshape(x.shape)
        return torch.sum(softmax.unsqueeze(2) * coords.to(softmax), dim=tuple(range(-len(spatial), 0)))
    ...
    def extract_patches(feature_map, starts, patch_size):
        ...
        spatial_indices = [indices.squeeze(2).unsqueeze(1) for indices in torch.split(bbox_indices, 1, 2)]
